...
var watchBilibili_config = global.config.bot.watchBilibiliry
...
async function watchBilibiliry(){
    await sleep(1000)
    watchBilibili_config = global.config.bot.watchBilibiliry
    for(let element of watchBilibili_config['bilibili_watchid']){
        if(!global.config.bot.watchBilibiliry.enable){
            set_watchbili_exit(3)
            await sleep(500)
            break
        }
...
export default watchBilibiliry;